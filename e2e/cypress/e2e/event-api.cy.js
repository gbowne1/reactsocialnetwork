...
describe("Event API tests", () => {
  ...
  it("GET - /api/events - should retrieve all existing events data", () => {
  ...
  it("GET - /api/event/:id - should retrieve specific event data", () => {
    ...
    cy.request({
      ...
      cy.request(`${apiUrl}/api/event/${eventId}`).then((res) => {
        ...
        expect(res.body.data.locationName).to.eq(testEvent.locationName);
  ...
  it("POST - /api/event - should register an event", () => {
    cy.request({
      method: "POST",
      url: `${apiUrl}/api/event/`,
      body: testEvent,
  ...
  it("PATCH - /api/event/:id - should update a specific event's data", () => {
    ...
    cy.request({
      ...
      cy.request({
        ...
        expect(res.body.data.locationName).to.eq(updatedEvent.locationName);
        expect(res.body.data.locationUrl).to.eq(updatedEvent.locationUrl);
  ...
  it("DELETE - /api/event/:id - shoudld delete a specific event", () => {
    // Register event and save it's id
    cy.request({
      method: "POST",
      url: `${apiUrl}/api/event/`,
