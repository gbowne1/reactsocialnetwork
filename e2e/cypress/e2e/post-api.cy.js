...
describe("Post API tests", () => {
  ...
  it("GET - /api/post/:id - should retrieve specific post data", () => {
  ...
  it("POST - /api/post/:id - should create a post", () => {
    cy.request(`${apiUrl}/api/post/${2}`).then((res) => {
      expect(res.body.message).to.eq("Post successfully retrieved!");
    });
  ...
  it("PATCH  - /api/post/:id - should update a specific post's data", () => {
    ...
    cy.request({
      ...
      cy.request({
        ...
        expect(res.body.data.accountImage).to.eq(updatedPost.accountImage);
        expect(res.body.data.accountName).to.eq(updatedPost.accountName);
  ...
  it("DELETE - /api/post/:id - should delete a specific post", () => {
    // Register post and save it's id
    cy.request({
      method: "POST",
      url: `${apiUrl}/api/post/`,
